...
static uint8_t
readmemb(uint32_t s, uint32_t a)
{
    if (is_new_biu)
        return readmemb_vx0(s, a);
    else {
        uint8_t ret;
...
static void
cpu_808x_access(int num, UNUSED(int bits))
{
    switch (num) {
        case  0:
        case 61:
...
interrupt(uint16_t addr)
{
    ...
    old_cs           = CS;
    cpu_808x_access(5, 16);
    new_ip = readmemw(0, cpu_state.eaaddr);
    wait_cycs(1, 0);
    cpu_state.eaaddr = (cpu_state.eaaddr + 2) & 0xffff;
    cpu_808x_access(6, 16);
    ...
    if (is_new_biu)
        ...
        cpu_808x_access(68, 16);
    set_ip(new_ip);
    if (is_new_biu) {
        biu_queue_flush();
        wait_cycs(4, 0);
    } else
        cpu_808x_access(41, 16);
    push(&old_ip);
}
...
execx86_instruction(void)
{
    ...
    uint16_t tempw_int, size, tempbp, lowbound, old_flags;
    ...
    if (is186) {
        switch (opcode) {
            ...
            case 0x6c ... 0x6d:    /* INM dst, DW/INS dst, DX */
                bits    = 8 << (opcode & 1);
                handled = 1;
                if (!repeating)
                    wait_cycs(2, 0);
            ...
            case 0x6e ... 0x6f:    /* OUTM DW, src/OUTS DX, src */
            ...
            case 0xc0 ... 0xc1:    /* rot imm8 */
                bits = 8 << (opcode & 1);
                do_mod_rm();
                if (cpu_mod == 3)
                    wait_cycs(1, 0);
                cpu_808x_access(53, bits);
                cpu_data = get_ea();
                cpu_src  = pfq_fetchb();
    ...
    if (!handled) {
        if (!is186 && !is_nec && (opcode >= 0x60) && (opcode <= 0x6f))
            opcode |= 0x10;
        ...
        switch (opcode) {
            ...
            case 0x86 ... 0x87:
                ...
                bits = 8 << (opcode & 1);
                do_mod_rm();
                cpu_808x_access(49, bits);
                cpu_data = get_ea();
                cpu_src  = get_reg(cpu_reg);
                set_reg(cpu_reg, cpu_data);
                wait_cycs(3, 0);
                cpu_808x_access(12, bits);
                set_ea(cpu_src);
                break;
            ...
            case 0x8f:             /* POPW */
                ...
                cpu_808x_access(24, 16);
                if (cpu_mod != 3)
                    wait_cycs(2, 0);
                cpu_data         = pop();
                cpu_state.eaaddr = cpu_src;
                wait_cycs(2, 0);
                cpu_808x_access(15, 16);
                seteaw(cpu_data);
                break;

            case 0x90 ... 0x97:    /* XCHG AX, rw */
                wait_cycs(1, 0);
            ...
            case 0x9a:             /* CALL FAR */
                ...
                if (is_new_biu)
                    ...
                    pfq_clear();
                    cpu_808x_access(31, 16);
                }
                push(&(CS));
                if (!is_new_biu)
                    cpu_808x_access(60, 16);
            ...
            case 0xa0 ... 0xa1:    /* MOV A, [iw] */
                bits = 8 << (opcode & 1);
                wait_cycs(1, 0);
                cpu_state.eaaddr = pfq_fetchw();
                cpu_808x_access(1, bits);
                set_accum(bits, readmem((ovr_seg ? *ovr_seg : ds)));
                wait_cycs(1, 0);
                break;
            case 0xa2 ... 0xa3:    /* MOV [iw], A */
                bits = 8 << (opcode & 1);
                wait_cycs(1, 0);
                cpu_state.eaaddr = pfq_fetchw();
                cpu_808x_access(7, bits);
            ...
            case 0xac ... 0xad:    /* LODS */
                ...
                if (in_rep != 0 && (opcode & 8) != 0)
                    wait_cycs(1, 0);
                cpu_808x_access(20, bits);
                lods(bits);
                if ((opcode & 8) == 0) {
                    cpu_808x_access(27, bits);
                    stos(bits);
                } else {
                    set_accum(bits, cpu_data);
                    if (in_rep != 0)
            ...
            case 0xae ... 0xaf:    /* SCAS */
                ...
                if ((opcode & 8) == 0) {
                    cpu_808x_access(21, bits);
                    lods(bits);
                    wait_cycs(1, 0);
                    cpu_dest = cpu_data;
                }
                cpu_808x_access(2, bits);
                cpu_state.eaaddr = DI;
                cpu_data         = readmem(es);
                DI               = string_increment(bits);
                cpu_src          = cpu_data;
            ...
            case 0xc8 ... 0xcb:
                ...
                if (is_new_biu)
                    ...
                    pfq_clear();
                cpu_808x_access(26, bits);
                new_ip = pop();
                wait_cycs(2, 0);
                if ((opcode & 8) == 0)
                    new_cs = CS;
                else {
                    cpu_808x_access(42, bits);
                    new_cs = pop();
                    if (opcode & 1)
                        wait_cycs(1, 0);
                }
                ...
                load_cs(new_cs);
                cpu_808x_access(72, bits);
                set_ip(new_ip);
                if (is_new_biu)
                    biu_queue_flush();
                break;

            case 0xc4 ... 0xc5:    /* LsS rw, rmd */
                do_mod_rm();
                bits = 16;
                cpu_808x_access(52, bits);
                read_ea(1, bits);
                cpu_state.regs[cpu_reg].w = cpu_data;
                cpu_808x_access(57, bits);
                read_ea2(bits);
                load_seg(cpu_data, (opcode & 0x01) ? &cpu_state.seg_ds : &cpu_state.seg_es);
                wait_cycs(1, 0);
                break;

            case 0xc6 ... 0xc7:    /* MOV rm, imm */
                bits = 8 << (opcode & 1);
                do_mod_rm();
                wait_cycs(1, 0);
                if (cpu_mod != 3)
            ...
            case 0xce:             /* INTO */
                ...
                break;
            case 0xcf:             /* IRET */
                cpu_808x_access(43, 8);
                new_ip = pop();
                wait_cycs(3, 0);
                cpu_808x_access(44, 8);
                new_cs = pop();
                load_cs(new_cs);
                cpu_808x_access(62, 8);
                set_ip(new_ip);
                if (is_new_biu)
                    biu_queue_flush();
                cpu_808x_access(45, 8);
                if (is_nec && cpu_md_write_disable)
                    cpu_state.flags = pop() | 0x8002;
                else
                    cpu_state.flags = pop() | 0x0002;
            ...
            case 0xec ... 0xef:
                ...
                if ((opcode & 2) == 0) {
                    ...
                    if ((opcode & 8) == 0)
                        cpu_808x_access(8, bits);
                    else
                        cpu_808x_access(9, bits);
                    if (opcode & 1)
            ...
            case 0xf0 ... 0xf1:    /*LOCK - F1 is alias*/
                in_lock = 1;
                wait_cycs(1, 0);
                completed = 0;
                break;
            ...
            case 0xf6 ... 0xf7:
                ...
                switch (rmdat & 0x38) {
                    ...
                    case 0x10: case 0x18:    /* NOT, NEG */
                        ...
                        cpu_808x_access(18, bits);
                        set_ea(cpu_data);
                        break;
                    case 0x20: case 0x28:    /* MUL, IMUL */
            ...
            case 0xfe ... 0xff:    /* misc */
                bits = 8 << (opcode & 1);
                do_mod_rm();
                cpu_808x_access(56, bits);
                read_ea(((rmdat & 0x38) == 0x18) || ((rmdat & 0x38) == 0x28), bits);
                switch (rmdat & 0x38) {
                    case 0x00: case 0x08:    /* INC rm, DEC rm */
                        cpu_dest = cpu_data;
                        cpu_src  = 1;
                        if ((rmdat & 0x38) == 0x00) {
                            cpu_data = cpu_dest + cpu_src;
                        ...
                        set_pzs(bits);
                        wait_cycs(2, 0);
                        cpu_808x_access(19, bits);
                        set_ea(cpu_data);
                        break;
                    case 0x10:               /* CALL rm */
                        cpu_data_opff_rm();
                        cpu_808x_access(63, bits);
                        wait_cycs(1, 0);
                        if (is_new_biu)
                            biu_suspend_fetch();
                        else
                        ...
                        if (is_new_biu)
                            biu_queue_flush();
                        wait_cycs(2, 0);
                        cpu_808x_access(35, bits);
                        push((uint16_t *) &(cpu_state.oldpc));
                        break;
                    case 0x18:               /* CALL rmd */
                        new_ip = cpu_data;
                        cpu_808x_access(58, bits);
                        read_ea2(bits);
                        if (!(opcode & 1))
                            cpu_data |= 0xff00;
                        new_cs = cpu_data;
                        cpu_808x_access(36, bits);
                        push(&(CS));
                        cpu_808x_access(64, bits);
                        if (!is_new_biu)
                            wait_cycs(4, 0);
                        cpu_state.oldpc = cpu_state.pc;
                        load_cs(new_cs);
                        ...
                        cpu_808x_access(37, bits);
                        push((uint16_t *) &(cpu_state.oldpc));
                        break;
                    case 0x20:               /* JMP rm */
                        cpu_data_opff_rm();
                        cpu_808x_access(65, bits);
                        set_ip(cpu_data);
                        if (is_new_biu)
                            biu_queue_flush();
                        break;
                    case 0x28:               /* JMP rmd */
                        new_ip = cpu_data;
                        cpu_808x_access(59, bits);
                        read_ea2(bits);
                        if (!(opcode & 1))
                            cpu_data |= 0xff00;
                        new_cs = cpu_data;
                        load_cs(new_cs);
                        cpu_808x_access(66, bits);
                        set_ip(new_ip);
                        if (is_new_biu)
                            biu_queue_flush();
                        break;
                    case 0x30: case 0x38:    /* PUSH rm */
                        if (cpu_mod != 3)
                            wait_cycs(1, 0);
                        cpu_808x_access(38, bits);
